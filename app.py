...
def search_tworld(query):
    try:
        ...
        logging.error(f"T world search error: {str(e)}")
        return []
...
def perform_search(query, df, sentence_transformer, translator):
    try:
        ...
        if not relevant_context.empty and relevant_context.iloc[0]['similarity'] > 0.7:
            ...
            logging.info("Found response in Fine-tuning data")
            return translate_text(response, translator), "Fine-tuning 데이터"
    except Exception as e:
        logging.error(f"Error in Fine-tuning data search: {str(e)}")
    ...
    try:
        ...
        if tworld_results:
            response = "\n".join(tworld_results)
            logging.info("Found response from T world website")
            return response, "T world 웹사이트"
    except Exception as e:
        logging.error(f"Error in T world website search: {str(e)}")
    ...
    try:
        ...
        if web_results:
            response = "\n".join(web_results)
            logging.info("Found response from web search")
            return translate_text(response, translator), "웹 검색"
    except Exception as e:
        logging.error(f"Error in web search: {str(e)}")

    logging.warning("No results found in any search method")
