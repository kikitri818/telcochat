...
def perform_search(query, df, sentence_transformer):
    ...
    if not relevant_context.empty and relevant_context.iloc[0]['similarity'] > 0.7:
        return relevant_context.iloc[0]['response'], "Fine-tuning 데이터"
...
def main():
    ...
    if user_input:
        response, source = perform_search(user_input, df, sentence_transformer)
